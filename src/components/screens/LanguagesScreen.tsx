...
import { PORTFOLIO_DATA, ProgrammingLanguage } from "@/data/portfolioData";
import { Code2, Terminal, CheckCircle2, Cpu, Sparkles } from "lucide-react";
  ...
  return (
    <div className="h-full w-full p-8 flex flex-col justify-between bg-[#0a0c12] text-zinc-100 font-sans select-none">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-zinc-800">
        ...
        <div className="flex items-center space-x-2 bg-emerald-500/10 px-3 py-1.5 rounded-lg border border-emerald-500/30 text-emerald-300 font-mono text-xs font-bold">
          <CheckCircle2 size={15} />
          <span>8 Core Languages Mastered</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-6 my-auto flex-1 py-4 overflow-y-auto">
        ...
        <div className="col-span-7 grid grid-cols-2 gap-3 pr-2">
          {languages.map((lang, idx) => {
            ...
            return (
              <button
                ...
                className={`p-3.5 rounded-xl text-left transition-all border-2 flex items-center justify-between cursor-pointer ${
                  isSelected
                    ? "bg-zinc-900 border-sky-400 shadow-lg shadow-sky-500/15 scale-[1.02]"
              ...
              >
                  ...
                  <div className="flex items-center space-x-2">
                    <span
                      ...
                      style={{
                        ...
                        boxShadow: `0 0 8px ${lang.accentColor}`,
                      }}
                    />
                    <span className="font-mono font-bold text-sm text-white">{lang.name}</span>
                  </div>
                ...
                <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-sky-500/20 text-sky-300 border border-sky-500/40 ml-2 flex-shrink-0">
                  {lang.level}
                </span>
              </button>
            );
        ...
        <div className="col-span-5 rounded-xl bg-[#06080c] border-2 border-zinc-800 p-5 flex flex-col justify-between shadow-xl">
          <div className="space-y-3">
            <div className="flex items-center justify-between pb-3 border-b border-zinc-800 text-xs">
              <div className="flex items-center space-x-2">
                <span className="font-mono font-bold text-sky-400 text-sm">{activeLang.name}</span>
                <span className="text-zinc-500">/</span>
                <span className="text-zinc-400 font-mono">architecture.snippet</span>
              </div>
              <span className="text-emerald-400 font-mono text-[11px] font-bold">● Production Tested</span>
            </div>
            ...
            <div className="p-3.5 rounded-lg bg-[#0b0e14] border border-zinc-800/90 font-mono text-xs text-zinc-200 shadow-inner overflow-x-auto">
